...
public class TipoViaGatewayApiTest {

    @Test
    public void list() {
        // given
        TipoViaGatewayApi gatewayApi = new TipoViaGatewayApi();
        String CONTENT = "{\"data\":100}";

        Response theResponse = JaxRsUtils.mockResponse(String.class, CONTENT, Response.Status.OK);

        TipoViaClient tipoViaClient = mock(TipoViaClient.class);
        when(tipoViaClient.list()).thenReturn(theResponse);

        ReflectionTestUtils.setField(gatewayApi, "client", tipoViaClient);

        // when
        Response response = gatewayApi.list();

        // then
        ApiUtils.assertThat(TipoViaGatewayApi.class, "list")
                .hasGetMapping("/tipo-via-gateway-api")
